## Use PUMS Data for Workforce Housing Spectrum Analysis
...
# Households with any worker in core workforce
pums_wf <- read_rds("data/pums/pums_faar.rds") |> 
  filter(
    core_workforce == TRUE,
    hh_income > 0
  )
...
pums_wf_hh <- pums_wf |> filter(SPORDER == 1)
