...
Incomes <- Incomes |> 
  mutate(locality = case_when(
    area_fips == "51033" ~ "Caroline County",
    area_fips == "51099" ~ "King George County",
    area_fips == "51137" ~ "Orange County",
    area_fips == "51177" ~ "Spotsylvania County",
    area_fips == "51179" ~ "Stafford County",
    area_fips == "51630" ~ "City of Fredericksburg",
...
# Modify function to get annual data 

average_annual <- Incomes |> 
  rename(Average_Weekly_Wage = annual_avg_wkly_wage, 
         Average_Annual_Salary = avg_annual_pay) |> 
  select(locality, year, Average_Weekly_Wage, Average_Annual_Salary) |>
  filter(year >= 2020)
...
costs_income <- faar_rent_annual |> 
  ...
  left_join(average_annual, by = c("locality", "year")) |> 
  left_join(faar_mls_annual, by = c("locality", "year")) |>
  filter(year >= 2020) |>
 mutate(
    rent_to_income = (rent * 12) / Average_Annual_Salary,
    price_to_income = msp / Average_Annual_Salary
  ) |> arrange(locality, year) 

costs_income <- costs_income |> rename(Median_Sales_Price = msp)
