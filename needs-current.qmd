...
Single-family homes dominate both the rental and ownership markets, leaving a gap in diverse housing types such as multifamily and senior-focused housing. There's a particular shortage of housing designed for single-person households, despite demographic trends showing an increase in smaller household sizes.
...
#| label: BPS-over-time
#| fig.cap: "Building Permits by Unit (2000-2020)"

years <- rep(seq(2000, 2020, by = 1), 3)
unit_type <- rep(c("1-unit", "2-4 units", "5+ units"), each = 21)

values <- c(
  # 1-unit values
  2800, 3800, 4000, 3800, 3800, 4500, 3000, 1900, 900, 1000, 1000,
  1200, 1600, 2100, 1400, 1900, 2200, 1900, 2200, 2500, 1000,
  # 2-4 units values
  0, 0, 0, 0, 150, 100, 50, 0, 0, 50, 0, 0, 0, 0, 0, 0, 0, 100, 0, 0, 0,
  # 5+ units values
  0, 0, 500, 300, 200, 300, 200, 0, 0, 0, 0, 200, 300, 0, 300, 500, 800, 600, 700, 1300, 1000
)

permits_data <- faar_cbps |> 
  rename(
    years = year,
    unit_type = type,
  ) |> 
  select(years, unit_type, units)
...
ggplot(permits_data, 
       aes(x = years, y = units, fill = unit_type)) +
  geom_col() +
  facet_wrap(~unit_type, scales = "free_y", nrow = 1) +
  ...
  labs(title = "Building Permits by Unit (2000-2020)",
       x = "",
       y = "",
...
Most housing units are concentrated in Fredericksburg, Stafford, and Spotsylvania, indicating a need for more balanced development across all localities in the region.
