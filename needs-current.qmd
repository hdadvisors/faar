...
faar_mls_annual <- read_rds("data/faar_mls_annual.rds") |>
  mutate(year = year(year), 
    ...
    locality = case_when(
      ...
      TRUE ~ locality  # keeps original value for any unmatched cases
    )
    ) |>
...
## Missing MLS data from 2016-2020 and income data 2023-2024
...
```{r}
#b25010 decade region vs supply 
...
```{r}
# Comparing household size trends with supply 

# Pulling supply data from general S/D
...
supply <- supply |>
  mutate(ami = factor(ami, levels = ami_levels)) |>
  arrange(ami)


hh_trends <- read_rds("data/fxburg_supply_demand.rds")
